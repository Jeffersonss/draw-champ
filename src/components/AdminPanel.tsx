import React, { useState, useEffect } from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Trash2, Edit2, Plus, Trophy, Users, Settings, Play, Square, RotateCcw, Home, ArrowLeft, ChevronRight, ArrowRight } from 'lucide-react';
import { toast } from 'sonner';
import { useNavigate } from 'react-router-dom';

export interface Club {
  id: string;
  name: string;
  logo: string;
  group?: number;
  position?: number;
}

export interface Tournament {
  name: string;
  format: 'groups' | 'knockout';
  totalClubs: number;
  groups?: number;
  clubsPerGroup?: number;
  clubs: Club[];
}

const AdminPanel = ({ onTournamentUpdate }: { onTournamentUpdate: (tournament: Tournament) => void }) => {
  const navigate = useNavigate();
  const [currentStep, setCurrentStep] = useState(1);
  // Remove showFinishDialog state
  const [tournament, setTournament] = useState<Tournament>({
    name: '',
    format: 'groups',
    totalClubs: 16,
    groups: 4,
    clubsPerGroup: 4,
    clubs: []
  });

  const [newClub, setNewClub] = useState({ name: '', logo: '' });
  const [editingClub, setEditingClub] = useState<Club | null>(null);
  const [drawStatus, setDrawStatus] = useState<'inactive' | 'active' | 'finished'>('inactive');
  const [selectedClub, setSelectedClub] = useState<Club | null>(null);

  useEffect(() => {
    const saved = localStorage.getItem('championship-tournament');
    const savedStatus = localStorage.getItem('championship-draw-status');
    
    if (saved) {
      const parsedTournament = JSON.parse(saved);
      setTournament(parsedTournament);
      onTournamentUpdate(parsedTournament);
    }
    
    // Always start at step 1 when loading the panel
    setCurrentStep(1);
    
    if (savedStatus) {
      setDrawStatus(savedStatus as 'inactive' | 'active' | 'finished');
    }
  }, [onTournamentUpdate]);

  // Modificar a função saveTournament para usar o toast com botão fechar e duração reduzida
  const saveTournament = (updatedTournament: Tournament) => {
    localStorage.setItem('championship-tournament', JSON.stringify(updatedTournament));
    setTournament(updatedTournament);
    onTournamentUpdate(updatedTournament);
    // Disparar evento customizado para notificar outras partes da aplicação
    window.dispatchEvent(new Event('tournamentUpdate'));
    
    // Toast com botão fechar e duração reduzida
    toast.success('Configuração salva com sucesso!', {
      duration: 2000,
      action: {
        label: 'Fechar',
        onClick: () => {}
      }
    });
  };

  const handleTournamentChange = (field: string, value: any) => {
    const updated = { ...tournament, [field]: value };
    
    if (field === 'format') {
      if (value === 'knockout') {
        updated.groups = undefined;
        updated.clubsPerGroup = undefined;
      } else {
        updated.groups = 4;
        updated.clubsPerGroup = 4;
        updated.totalClubs = 16;
      }
    }
    
    if (field === 'groups' || field === 'clubsPerGroup') {
      updated.totalClubs = (updated.groups || 1) * (updated.clubsPerGroup || 1);
    }
    
    saveTournament(updated);
    
    // Only show toast for name changes when field loses focus
    if (field === 'name') {
      toast('Configuração salva', {
        duration: 3000,
        action: {
          label: 'Fechar',
          onClick: () => {}
        }
      });
    }
  };

  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        setNewClub({ ...newClub, logo: event.target?.result as string });
      };
      reader.readAsDataURL(file);
    }
  };

  const addClub = () => {
    if (!newClub.name.trim()) {
      toast.error('Nome do clube é obrigatório');
      return;
    }

    if (tournament.clubs.length >= tournament.totalClubs) {
      toast.error(`Máximo de ${tournament.totalClubs} clubes permitido`);
      return;
    }

    const club: Club = {
      id: Date.now().toString(),
      name: newClub.name,
      logo: newClub.logo || `https://ui-avatars.com/api/?name=${newClub.name}&background=3b82f6&color=fff`
    };

    const updated = {
      ...tournament,
      clubs: [...tournament.clubs, club]
    };

   saveTournament(updated);
    setNewClub({ name: '', logo: '' });
    // Manter apenas este toast:
    toast(`${club.name} adicionado!`, {
        duration: 3000,
        action: {
            label: 'Fechar',
            onClick: () => {}
        }
    });
    
  };

  const updateClub = () => {
    if (!editingClub || !editingClub.name.trim()) return;

    const updated = {
      ...tournament,
      clubs: tournament.clubs.map(club => 
        club.id === editingClub.id ? editingClub : club
      )
    };

    // Salvar sem mostrar toast duplicado
    localStorage.setItem('championship-tournament', JSON.stringify(updated));
    setTournament(updated);
    onTournamentUpdate(updated);
    window.dispatchEvent(new Event('tournamentUpdate'));
    
    setEditingClub(null);
    toast.success('Clube atualizado!', {
      duration: 2000,
      action: {
        label: 'Fechar',
        onClick: () => {}
      }
    });
  };

  const removeClub = (id: string) => {
    const updated = {
      ...tournament,
      clubs: tournament.clubs.filter(club => club.id !== id)
    };

    // Salvar sem mostrar toast duplicado
    localStorage.setItem('championship-tournament', JSON.stringify(updated));
    setTournament(updated);
    onTournamentUpdate(updated);
    window.dispatchEvent(new Event('tournamentUpdate'));
    
    toast.success('Clube removido!', {
      duration: 2000,
      action: {
        label: 'Fechar',
        onClick: () => {}
      }
    });
  };

  const startDraw = () => {
    if (!tournament.name.trim()) {
      toast.error('Nome do campeonato é obrigatório');
      return;
    }
    if (tournament.clubs.length !== tournament.totalClubs) {
      toast.error(`É necessário cadastrar exatamente ${tournament.totalClubs} clubes para iniciar o sorteio`);
      return;
    }
    
    setDrawStatus('active');
    setCurrentStep(3);
    localStorage.setItem('championship-draw-status', 'active');
    toast.success('Sorteio iniciado! Agora você pode definir os grupos e posições.');
  };

  const finishDraw = () => {
    // Verificar se todos os times têm grupo e posição definidos
    const unassignedClubs = tournament.clubs.filter(club => !club.group || !club.position);
    if (unassignedClubs.length > 0) {
      toast.error('Todos os times precisam ter grupo e posição definidos antes de finalizar o sorteio');
      return;
    }

    setDrawStatus('finished');
    setCurrentStep(1); // Reset to step 1
    localStorage.setItem('championship-draw-status', 'finished');
    localStorage.removeItem('championship-tournament');
    toast.success('Sorteio finalizado!');
  };

  const restartDraw = () => {
    setDrawStatus('inactive');
    setCurrentStep(1);
    localStorage.setItem('championship-draw-status', 'inactive');
    localStorage.removeItem('championship-groups');

    // Limpar grupos e posições dos times
    const updatedTournament = {
      ...tournament,
      clubs: tournament.clubs.map(club => ({
        ...club,
        group: undefined,
        position: undefined
      }))
    };
    saveTournament(updatedTournament);

    toast.success('Sorteio reiniciado!');
  };

  const handleClubPositionUpdate = () => {
    if (!selectedClub || !selectedClub.group || !selectedClub.position) {
      toast.error('Selecione um time e defina o grupo e a posição');
      return;
    }

    // Verificar se já existe um time na posição selecionada
    const existingClub = tournament.clubs.find(
      club => 
        club.id !== selectedClub.id && 
        club.group === selectedClub.group && 
        club.position === selectedClub.position
    );

    if (existingClub) {
      toast.error(`Já existe um time na posição ${selectedClub.position} do grupo ${selectedClub.group}`);
      return;
    }

    const updated = {
      ...tournament,
      clubs: tournament.clubs.map(club =>
        club.id === selectedClub.id ? { ...club, group: selectedClub.group, position: selectedClub.position } : club
      )
    };

    // Salvar sem mostrar toast duplicado
    localStorage.setItem('championship-tournament', JSON.stringify(updated));
    setTournament(updated);
    onTournamentUpdate(updated);
    window.dispatchEvent(new Event('tournamentUpdate'));
    
    setSelectedClub(null);
    toast.success('Posição do time atualizada!');
  };

  const nextStep = () => {
    if (currentStep === 1 && !tournament.name.trim()) {
      toast.error('Nome do campeonato é obrigatório');
      return;
    }
    if (currentStep === 2 && tournament.clubs.length !== tournament.totalClubs) {
      toast.error(`É necessário cadastrar exatamente ${tournament.totalClubs} clubes`);
      return;
    }
    setCurrentStep(prev => prev + 1);
  };

  const prevStep = () => {
    setCurrentStep(prev => prev - 1);
  };

  return (
    <div className="min-h-screen p-6 relative overflow-hidden">
      {/* Background */}
      <div 
        className="fixed inset-0 z-0"
        style={{
          backgroundImage: `url('/backgrounds/background.webp')`,
          backgroundSize: 'cover',
          backgroundPosition: 'center',
          backgroundAttachment: 'fixed'
        }}
      />
      
      <div className="relative z-10 max-w-6xl mx-auto">
        {/* Navigation and Header */}
        <div className="flex justify-between items-center mb-6">
          <Button
            onClick={() => navigate('/')}
            variant="outline"
            className="text-blue-800 border-blue-400 hover:bg-blue-400/10"
          >
            <Home className="h-4 w-4 mr-2" />
            Página Inicial
          </Button>
        </div>
          
        <div className="admin-header text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            <Trophy className="h-8 w-8" />
            <h1 className="text-3xl font-bold">Painel Administrativo</h1>
          </div>
          <p className="text-white/90">Configure seu campeonato e gerencie os clubes participantes</p>
          
          {/* Steps Indicator */}
          <div className="flex justify-center gap-4 mt-6">
            <div className={`step ${currentStep >= 1 ? 'active' : ''}`}>1. Configuração</div>
            <div className={`step ${currentStep >= 2 ? 'active' : ''}`}>2. Times</div>
            <div className={`step ${currentStep >= 3 ? 'active' : ''}`}>3. Sorteio</div>
          </div>
        </div>

        {/* Step 1: Tournament Configuration */}
        {currentStep === 1 && (
          <>
            <Card className="championship-card">
              <CardHeader>
              <CardTitle className="text-2xl font-bold text-center">Configuração do Campeonato</CardTitle>
            </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  <Label htmlFor="tournament-name">Nome do Campeonato</Label>
                  <Input
                    id="tournament-name"
                    value={tournament.name}
                    onChange={(e) => handleTournamentChange('name', e.target.value)}
                    placeholder="Ex: Copa do Mundo 2024"
                    className="mt-1"
                  />
                </div>

                <div>
                  <Label>Formato</Label>
                  <Select
                    value={tournament.format}
                    onValueChange={(value) => handleTournamentChange('format', value)}
                  >
                    <SelectTrigger className="mt-1">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="groups">Fase de Grupos</SelectItem>
                      <SelectItem value="knockout">Mata-mata</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                {tournament.format === 'groups' && (
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label>Número de Grupos</Label>
                        <Input
                          type="number"
                          min={2}
                          max={8}
                          value={tournament.groups}
                          onChange={(e) => handleTournamentChange('groups', parseInt(e.target.value))}
                          className="mt-1"
                        />
                      </div>
                      <div>
                        <Label>Clubes por Grupo</Label>
                        <Input
                          type="number"
                          min={2}
                          max={8}
                          value={tournament.clubsPerGroup}
                          onChange={(e) => handleTournamentChange('clubsPerGroup', parseInt(e.target.value))}
                          className="mt-1"
                        />
                      </div>
                    </div>
                    <div className="text-sm text-muted-foreground bg-championship-subtle p-3 rounded-lg">
                      Total de clubes: {tournament.totalClubs}
                    </div>
                  </>
                )}

                {tournament.format === 'knockout' && (
                  <div>
                    <Label>Total de Clubes</Label>
                    <Select
                      value={tournament.totalClubs.toString()}
                      onValueChange={(value) => handleTournamentChange('totalClubs', parseInt(value))}
                    >
                      <SelectTrigger className="mt-1">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="8">8 (Quartas)</SelectItem>
                        <SelectItem value="16">16 (Oitavas)</SelectItem>
                        <SelectItem value="32">32 (Primeira Fase)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                )}


              </CardContent>
            </Card>
            <div className="flex justify-end mt-6">
              {currentStep < 3 && (
                <Button onClick={nextStep} className="bg-championship-primary hover:bg-championship-primary/90">
                  Próximo Passo <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              )}
            </div>
          </>
        )}

        {/* Step 2: Club Management */}
        {currentStep === 2 && (
          <>
            <>
              <Card className="championship-card">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Users className="h-5 w-5 text-championship-primary" />
                    Gerenciar Clubes ({tournament.clubs.length}/{tournament.totalClubs})
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-4">
                    <div>
                      <Label>Nome do Clube</Label>
                      <Input
                        value={editingClub?.name || newClub.name}
                        onChange={(e) => 
                          editingClub 
                            ? setEditingClub({...editingClub, name: e.target.value})
                            : setNewClub({...newClub, name: e.target.value})
                        }
                        placeholder="Nome do clube"
                        className="mt-1"
                      />
                    </div>
                    <div>
                      <Label>Logo do Clube</Label>
                      <input
                        type="file"
                        accept="image/*"
                        onChange={handleLogoUpload}
                        className="mt-1 block w-full text-sm text-muted-foreground file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-championship-primary file:text-white hover:file:bg-championship-primary-dark"
                      />
                    </div>
                    <Button
                      onClick={editingClub ? updateClub : addClub}
                      className="championship-button w-full"
                      disabled={tournament.clubs.length >= tournament.totalClubs && !editingClub}
                    >
                      <Plus className="h-4 w-4 mr-2" />
                      {editingClub ? 'Atualizar' : 'Adicionar'} Clube
                    </Button>
                  </div>
                </CardContent>
              </Card>

              {/* Clubs List */}
              {tournament.clubs.length > 0 && (
                <Card className="championship-card mt-8">
                  <CardHeader>
                    <CardTitle>Clubes Cadastrados</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                      {tournament.clubs.map((club) => (
                        <div key={club.id} className="group-card p-4">
                          <div className="flex items-center gap-3">
                            <img
                              src={club.logo}
                              alt={club.name}
                              className="w-10 h-10 rounded-full object-cover"
                            />
                            <div className="flex-1 min-w-0">
                              <h3 className="font-medium truncate">{club.name}</h3>
                            </div>
                            <div className="flex gap-2">
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setEditingClub(club)}
                                className="h-8 w-8 p-0"
                              >
                                <Edit2 className="h-4 w-4" />
                              </Button>
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => removeClub(club.id)}
                                className="h-8 w-8 p-0 text-destructive hover:text-destructive"
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
                            </div>
                          </div>
                        </div>
                      ))}
                    </div>
                  </CardContent>
                </Card>
              )}
            </>

            {/* Navigation Buttons for Step 2 */}
            <div className="flex justify-between mt-6">
              <Button onClick={prevStep} variant="outline" className="text-blue-800 hover:text-blue-950 hover:border-blue-500">
                <ArrowLeft className="mr-2 h-4 w-4" /> Passo Anterior
              </Button>
              <Button onClick={nextStep} className="bg-championship-primary hover:bg-championship-primary/90">
                Próximo Passo <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
          </>
        )}


        {/* Step 3: Draw Management */}
        {currentStep === 3 && (
          <>
            {/* Draw Control Buttons */}
            <div className="flex justify-center gap-4 mb-8">
              <Button onClick={prevStep} variant="outline" className="text-blue-800 hover:text-blue-950 hover:border-blue-500">
                <ArrowLeft className="mr-2 h-4 w-4" /> Passo Anterior
              </Button>
              {drawStatus === 'inactive' && (
                <Button
                  onClick={startDraw}
                  className="championship-button px-6"
                  disabled={tournament.clubs.length !== tournament.totalClubs || !tournament.name.trim()}
                >
                  <Play className="h-4 w-4 mr-2" />
                  Iniciar Sorteio
                </Button>
              )}
              
              {drawStatus === 'active' && (
                <div className="flex gap-4">
                  <Button
                    onClick={finishDraw}
                    className="championship-button px-6"
                  >
                    <Square className="h-4 w-4 mr-2" />
                    Finalizar Sorteio
                  </Button>
                  <Button
                    onClick={restartDraw}
                    variant="outline"
                    className="text-blue-800 hover:text-blue-950 hover:border-blue-500"
                  >
                    <RotateCcw className="h-4 w-4 mr-2" />
                    Reiniciar Sorteio
                  </Button>
                </div>
              )}
              
              {drawStatus === 'finished' && (
                <Button
                  onClick={restartDraw}
                  variant="outline"
                  className="text-blue-800 hover:text-blue-950 hover:border-blue-500"
                >
                  <RotateCcw className="h-4 w-4 mr-2" />
                  Reiniciar Sorteio
                </Button>
              )}
            </div>

            {/* Club Position Assignment */}
            {drawStatus === 'active' && (
              <Card className="championship-card">
                <CardHeader>
                  <CardTitle>Definir Posições dos Times</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-6">
                    {/* Club Selection */}
                    <div>
                      <Label>Selecionar Time</Label>
                      <Select
                        value={selectedClub?.id || ''}
                        onValueChange={(value) => {
                          const club = tournament.clubs.find(c => c.id === value);
                          setSelectedClub(club || null);
                        }}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Escolha um time" />
                        </SelectTrigger>
                        <SelectContent>
                          {tournament.clubs.map(club => (
                            <SelectItem key={club.id} value={club.id}>
                              {club.name} {club.group ? `(Grupo ${String.fromCharCode(64 + club.group)}, Pos. ${club.position})` : ''}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </div>

                    {/* Group Selection */}
                    {selectedClub && tournament.format === 'groups' && (
                      <div>
                        <Label>Grupo</Label>
                        <Select
                          value={selectedClub.group?.toString() || ''}
                          onValueChange={(value) => setSelectedClub(prev => prev ? { ...prev, group: parseInt(value) } : null)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Selecione o grupo" />
                          </SelectTrigger>
                          <SelectContent>
                            {Array.from({ length: tournament.groups || 0 }, (_, i) => i + 1).map(group => (
                              <SelectItem key={group} value={group.toString()}>
                                Grupo {String.fromCharCode(64 + group)}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
                    )}

                    {/* Position Selection */}
                    {selectedClub && selectedClub.group && (
                      <div>
                        <Label>Posição no Grupo</Label>
                        <Select
                          value={selectedClub.position?.toString() || ''}
                          onValueChange={(value) => setSelectedClub(prev => prev ? { ...prev, position: parseInt(value) } : null)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Selecione a posição" />
                          </SelectTrigger>
                          <SelectContent>
                            {Array.from({ length: tournament.clubsPerGroup || tournament.totalClubs }, (_, i) => i + 1).map(pos => (
                              <SelectItem key={pos} value={pos.toString()}>
                                Posição {pos}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
                    )}

                    <Button
                      onClick={handleClubPositionUpdate}
                      className="championship-button w-full"
                      disabled={!selectedClub || !selectedClub.group || !selectedClub.position}
                    >
                      Definir Posição
                    </Button>
                  </div>

                  {/* Current Assignments */}
                  <div className="mt-8">
                    <h3 className="text-lg font-semibold mb-4">Posições Definidas</h3>
                    <div className="grid gap-6">
                      {Array.from({ length: tournament.groups || 1 }, (_, i) => i + 1).map(group => (
                        <div key={group} className="space-y-2">
                          <h4 className="font-medium">Grupo {String.fromCharCode(64 + group)}</h4>
                          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            {Array.from({ length: tournament.clubsPerGroup || tournament.totalClubs }, (_, i) => i + 1).map(position => {
                              const assignedClub = tournament.clubs.find(club => club.group === group && club.position === position);
                              return (
                                <div key={position} className="p-3 bg-white/10 rounded-lg">
                                  <div className="text-sm text-muted-foreground">Posição {position}</div>
                                  {assignedClub ? (
                                    <div className="flex items-center gap-2 mt-1">
                                      <img
                                        src={assignedClub.logo}
                                        alt={assignedClub.name}
                                        className="w-6 h-6 rounded-full"
                                      />
                                      <span className="font-medium">{assignedClub.name}</span>
                                    </div>
                                  ) : (
                                    <div className="text-sm italic text-muted-foreground mt-1">Vazio</div>
                                  )}
                                </div>
                              );
                            })}
                          </div>
                        </div>
                      ))}
                    </div>
                  </div>
                </CardContent>
              </Card>
            )}
          </>
        )}
      
      </div>
    </div>
  );
};

export default AdminPanel;